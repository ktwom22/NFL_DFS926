...
class DraftKingsOptimizer:
    def __init__(self, player_df: pd.DataFrame, **kwargs):
        ...
        self.df["position"] = self.df["position"].astype(str).str.upper().str.strip()
        self.df["position"] = self.df["position"].replace({"D": "DST", "DEF": "DST"})
        ...
        self.df["opportunity_ceiling"] = self.df.apply(self._calculate_opportunity_ceiling, axis=1)
    ...
    def apply_user_guide(self, boosts: dict = None, locks: list = None, fades: list = None):
        if boosts:
            for player, mult in boosts.items():
                self.df.loc[self.df["name"] == player, "user_boost"] = mult

        self.df["adjusted_points"] = self.df["opportunity_ceiling"] * self.df["user_boost"]
    ...
    def optimize_showdown_multi(
        ...
        **kwargs
        ...
        cpt = {p["pulp_id"]: pulp.LpVariable(f"cpt_{p['pulp_id']}", cat="Binary") for p in players}
        flex = {p["pulp_id"]: pulp.LpVariable(f"flex_{p['pulp_id']}", cat="Binary") for p in players}

        prob += pulp.lpSum(
            (p["salary"] * 1.5 * cpt[p["pulp_id"]]) + (p["salary"] * flex[p["pulp_id"]])
            for p in players
        ) <= salary_ceiling
        ...
        teams = list(set(p["team"] for p in players if p.get("team") and p["team"] != "UNK"))
        if len(teams) >= 2:
            prob += pulp.lpSum(cpt[p["pulp_id"]] + flex[p["pulp_id"]] for p in players if p["team"] == teams[0]) >= 1
            prob += pulp.lpSum(cpt[p["pulp_id"]] + flex[p["pulp_id"]] for p in players if p["team"] == teams[1]) >= 1

        for t in teams:
            team_qbs = [p for p in players if p["team"] == t and p["position"] == "QB"]
            if len(team_qbs) > 1:
                prob += pulp.lpSum(cpt[p["pulp_id"]] + flex[p["pulp_id"]] for p in team_qbs) <= 1

            team_tes = [p for p in players if p["team"] == t and p["position"] == "TE"]
            if len(team_tes) > 1:
                prob += pulp.lpSum(cpt[p["pulp_id"]] + flex[p["pulp_id"]] for p in team_tes) <= 1

        kickers = [p for p in players if p["position"] == "K"]
        if kickers:
            prob += pulp.lpSum(cpt[p["pulp_id"]] + flex[p["pulp_id"]] for p in kickers) <= 1

        for p in players:
            if p["position"] == "DST":
                opp_qbs = [q for q in players if q["position"] == "QB" and q["team"] != p["team"]]
                for qb in opp_qbs:
                    prob += cpt[qb["pulp_id"]] + flex[qb["pulp_id"]] + cpt[p["pulp_id"]] + flex[p["pulp_id"]] <= 1

                opp_kickers = [k for k in players if k["position"] == "K" and k["team"] != p["team"]]
                for k in opp_kickers:
                    prob += cpt[k["pulp_id"]] + flex[k["pulp_id"]] + cpt[p["pulp_id"]] + flex[p["pulp_id"]] <= 1

        fav = favorite_team if favorite_team in teams else (teams[0] if teams else None)
        und = underdog_team if underdog_team in teams else (teams[1] if len(teams) > 1 else None)

        if game_script == "shootout":
            qbs = [p for p in players if p["position"] == "QB"]
            if qbs:
                prob += pulp.lpSum(cpt[p["pulp_id"]] + flex[p["pulp_id"]] for p in qbs) >= 1
                prob += pulp.lpSum(cpt[p["pulp_id"]] + flex[p["pulp_id"]] for p in qbs) <= 2
            for p in [p for p in players if p["position"] in ["K", "DST"]]:
                prob += cpt[p["pulp_id"]] + flex[p["pulp_id"]] == 0
            for qb in qbs:
                team_wrs = [p for p in players if p["team"] == qb["team"] and p["position"] == "WR"]
                if team_wrs:
                    prob += pulp.lpSum(cpt[p["pulp_id"]] + flex[p["pulp_id"]] for p in team_wrs) >= (cpt[qb["pulp_id"]] + flex[qb["pulp_id"]])
            for p in players:
                if p["salary"] < 6000:
                    prob += cpt[p["pulp_id"]] == 0

        elif game_script == "5_1_onslaught" and fav and und:
            prob += pulp.lpSum(cpt[p["pulp_id"]] + flex[p["pulp_id"]] for p in players if p["team"] == fav) == 5
            prob += pulp.lpSum(cpt[p["pulp_id"]] + flex[p["pulp_id"]] for p in players if p["team"] == und) == 1
            prob += pulp.lpSum(cpt[p["pulp_id"]] for p in players if p["team"] == fav) == 1
            for p in players:
                if p["salary"] < 4000:
                    prob += cpt[p["pulp_id"]] == 0
            ...
            prob += pulp.lpSum(cpt[p["pulp_id"]] + flex[p["pulp_id"]] for p in qbs) <= 1
            prob += pulp.lpSum(cpt[p["pulp_id"]] + flex[p["pulp_id"]] for p in k_dst) >= 2
            for p in players:
                if p["salary"] < 3000:
                    prob += cpt[p["pulp_id"]] == 0

        elif game_script == "underdog_chase" and und:
            und_wrs = [p for p in players if p["team"] == und and p["position"] == "WR"]
            und_qbs = [p for p in players if p["team"] == und and p["position"] == "QB"]
            if und_wrs:
            ...
            for p in players:
                if p["position"] in ["WR", "TE"]:
                    team_qb = next((q for q in players if q["position"] == "QB" and q["team"] == p["team"]), None)
                    if team_qb:
                        prob += cpt[team_qb["pulp_id"]] + flex[team_qb["pulp_id"]] <= 1 - cpt[p["pulp_id"]]

        else:
            if len(teams) >= 2:
                prob += pulp.lpSum(cpt[p["pulp_id"]] + flex[p["pulp_id"]] for p in players if p["team"] == teams[0]) >= 2
                prob += pulp.lpSum(cpt[p["pulp_id"]] + flex[p["pulp_id"]] for p in players if p["team"] == teams[1]) >= 2
            for p in players:
                if p["salary"] < 5000:
                    prob += cpt[p["pulp_id"]] == 0
        ...
        for line_num in range(1, n_lineups + 1):
            ...
            if randomness > 0 and line_num > 1:
                ...
                for p in players:
                    ...
                    perturbed_pts[p["pulp_id"]] = max(0.5, random.gauss(p["adjusted_points"], p["adjusted_points"] * vol))
            else:
                perturbed_pts = {p["pulp_id"]: p["adjusted_points"] for p in players}

            prob.objective = pulp.lpSum(
                (perturbed_pts[p["pulp_id"]] * 1.5 * cpt[p["pulp_id"]]) + (perturbed_pts[p["pulp_id"]] * flex[p["pulp_id"]])
                for p in players
            )

            solver_status = prob.solve(pulp.PULP_CBC_CMD(msg=False, keepFiles=False))
            ...
            flex_r = ldf[ldf["Slot"] == "FLEX"].sort_values(by="Salary", ascending=False)
    ...
    def optimize_classic_multi(
        ...
        **kwargs
        ...
        - Allows dual-TE only when an elite TE is drafted (hurdle model)
        - Windows-safe identifiers & CBC execution
        """
        ...
        if len(df_classic) < 9:
            raise ValueError(f"Player pool has only {len(df_classic)} players (minimum 9 required).")
        ...
        if len(rbs) < 2 or len(wrs) < 3 or len(tes) < 1:
            raise ValueError("Insufficient skill players (RB/WR/TE) to fill a legal roster.")

        prob = pulp.LpProblem("DK_NFL_Classic_Multi", pulp.LpMaximize)
        x = {p["pulp_id"]: pulp.LpVariable(f"p_{p['pulp_id']}", cat="Binary") for p in players}

        # 1. Salary Cap & Total Roster Count
        prob += pulp.lpSum(p["salary"] * x[p["pulp_id"]] for p in players) <= salary_cap
        ...
        # TE Hurdle: allow dual TE only if at least one elite TE is present
        if contest_type in ["milly_maker", "limited_entry"]:
            elite_tes = [
                p["pulp_id"] for p in tes
                if p["adjusted_points"] >= 14.0 or p["salary"] >= 6000
            ]
            if elite_tes:
                prob += pulp.lpSum(x[p["pulp_id"]] for p in tes) <= 1 + pulp.lpSum(x[pid] for pid in elite_tes)
        ...
        teams = set(p["team"] for p in players if p.get("team") and p["team"] != "UNK")
        for t in teams:
        ...
        for qb in qbs:
            ...
            teammates = [
                p["pulp_id"] for p in players
                if p["team"] == qb_team and p["position"] in ["WR", "TE"]
            ]
            bring_backs = [
                p["pulp_id"] for p in players
                if opp_team != "UNK" and p["team"] == opp_team and p["position"] in ["WR", "RB", "TE"]
            ]

            if stack_style == "game_stack":
                if teammates:
                    prob += pulp.lpSum(x[pid] for pid in teammates) >= x[qb["pulp_id"]]
                if bring_backs:
                    prob += pulp.lpSum(x[pid] for pid in bring_backs) >= x[qb["pulp_id"]]

            elif stack_style == "double_stack":
                if len(teammates) >= 2:
                    prob += pulp.lpSum(x[pid] for pid in teammates) >= 2 * x[qb["pulp_id"]]
                elif teammates:
                    prob += pulp.lpSum(x[pid] for pid in teammates) >= x[qb["pulp_id"]]

            elif stack_style == "double_game_stack":
                if len(teammates) >= 2:
                    prob += pulp.lpSum(x[pid] for pid in teammates) >= 2 * x[qb["pulp_id"]]
                elif teammates:
                    prob += pulp.lpSum(x[pid] for pid in teammates) >= x[qb["pulp_id"]]
        ...
        for line_num in range(1, n_lineups + 1):
            if line_num > 1:
                for p in players:
                    if p["name"] not in self.locks:
                        if p["position"] == "QB" and player_counts[p["pulp_id"]] >= max_allowed_qb:
                            prob += x[p["pulp_id"]] == 0
                        elif p["position"] != "QB" and player_counts[p["pulp_id"]] >= max_allowed_skill:
                            prob += x[p["pulp_id"]] == 0
            ...
            prob.objective = pulp.lpSum(perturbed_pts[p["pulp_id"]] * x[p["pulp_id"]] for p in players)

            solver_status = prob.solve(pulp.PULP_CBC_CMD(msg=False, keepFiles=False))
            if pulp.LpStatus[solver_status] != "Optimal":
                break

            current_pids = [p["pulp_id"] for p in players if pulp.value(x[p["pulp_id"]]) == 1]
            for pid in current_pids:
                player_counts[pid] += 1

            selected = [p for p in players if p["pulp_id"] in current_pids]
            selected = sorted(selected, key=lambda p: (p["position"] != "QB", p["position"] != "DST", -p["salary"]))
            ...
            slot_order = {"QB": 0, "RB1": 1, "RB2": 2, "WR1": 3, "WR2": 4, "WR3": 5, "TE": 6, "FLEX": 7, "DST": 8}
